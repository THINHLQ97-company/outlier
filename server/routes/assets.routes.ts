...
function withFlags(row: any, me: string) {
  return { ...row, isMine: row.owner === me };
}
...
export function registerAssetRoutes(app: Express) {
  ...
  app.get("/api/assets", requireAuth, async (req, res) => {
    ...
    try {
      ...
      res.json(rows.map((r) => withFlags(r, me)));
    } catch (e: any) {
  ...
  // DELETE /api/assets/:id — chỉ chủ sở hữu (xoá cả file ảnh nội bộ).
  app.delete("/api/assets/:id", requireAuth, async (req, res) => {
    ...
    try {
      ...
      const [existing] = await db.select().from(assets).where(eq(assets.id, id));
      if (!existing) return res.status(404).json({ error: "Không tìm thấy asset." });
      if (existing.owner !== me) return res.status(403).json({ error: "Chỉ chủ sở hữu mới xoá được asset này." });
