...
// opacity ~60%. Toạ độ text box lưu dạng phân số (0-1) của canvas gốc
// (BASE_SIZE) để export PNG cuối luôn đúng tỉ lệ bất kể kích thước hiển thị.
...
export default function TextOverlayEditor({ imageUrl, initialOverlay, watermarkOptions, onExport, exporting }: Props) {
  ...
  const scale = DISPLAY_SIZE / BASE_SIZE;
  ...
  function draw() {
    ...
    if (img) ctx.drawImage(img, 0, 0, DISPLAY_SIZE, DISPLAY_SIZE);
    else {
    ...
    textBoxes.forEach((box) => {
      ctx.save();
      const x = box.x * DISPLAY_SIZE;
      const y = box.y * DISPLAY_SIZE;
    ...
    ctx.fillRect(DISPLAY_SIZE - wmWidth - 28, DISPLAY_SIZE - 22 * scale - 22, wmWidth + 16, 22 * scale + 12);
    ctx.fillStyle = "#ffffff";
    ctx.fillText(watermarkBrand, DISPLAY_SIZE - wmWidth - 20, DISPLAY_SIZE - 16);
    ctx.restore();
  }
  ...
  function handleMouseDown(e: React.MouseEvent<HTMLCanvasElement>) {
    ...
    if (hit) {
      ...
      dragState.current = { id: hit.id, offsetX: px - hit.x * DISPLAY_SIZE, offsetY: py - hit.y * DISPLAY_SIZE };
    } else {
      setSelectedId(null);
    }
  }
  ...
  function handleMouseMove(e: React.MouseEvent<HTMLCanvasElement>) {
    ...
    const nx = Math.min(1, Math.max(0, (px - offsetX) / DISPLAY_SIZE));
    const ny = Math.min(1, Math.max(0.03, (py - offsetY) / DISPLAY_SIZE));
  ...
  function handleExport() {
    ...
    off.width = BASE_SIZE;
    off.height = BASE_SIZE;
    const ctx = off.getContext("2d")!;
    const img = imgRef.current;
    if (img) ctx.drawImage(img, 0, 0, BASE_SIZE, BASE_SIZE);
    else {
      ctx.fillStyle = "#e7e5e4";
      ctx.fillRect(0, 0, BASE_SIZE, BASE_SIZE);
    }
    textBoxes.forEach((box) => {
      const x = box.x * BASE_SIZE;
      const y = box.y * BASE_SIZE;
    ...
    const wmWidth = ctx.measureText(watermarkBrand).width;
    ctx.fillStyle = "rgba(0,0,0,0.35)";
    ctx.fillRect(BASE_SIZE - wmWidth - 56, BASE_SIZE - 46 - 40, wmWidth + 32, 46 + 24);
    ctx.fillStyle = "#ffffff";
    ctx.fillText(watermarkBrand, BASE_SIZE - wmWidth - 40, BASE_SIZE - 34);
    ctx.globalAlpha = 1;

    const dataUrl = off.toDataURL("image/png");
    onExport({ textBoxes, watermarkBrand }, dataUrl);
  }
